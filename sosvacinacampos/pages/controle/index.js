      ...
      <div className={styles.container}>
        ...
        <main className={styles.main}>
          ...
          <table className={'table'}>
            ...
            <tbody>
              {vaccined.sort((a,b) => a.data - b.data).map(item => (
                <tr key={item.id}>
                  <th scope="row" style={{textAlign: 'center'}}>{item.data && (new Date(item.data._seconds*1000)).toLocaleDateString()}</th>
                  <td style={{textAlign: 'center'}}>{item.qtd_planejado}</td>
